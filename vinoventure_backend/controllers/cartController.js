...
exports.addToCart = async (req, res) => {
    ...
    if (!package_id || !quantity) {
        return res.status(400).json({ message: 'Package ID und Menge sind erforderlich' });
    }

    try {
        const [cartResult] = await db.query('SELECT cart_id FROM shopping_cart WHERE user_id = ?', [userId]);

        let cartId;
        if (cartResult.length === 0) {
            const [createCartResult] = await db.query('INSERT INTO shopping_cart (user_id) VALUES (?)', [userId]);
        ...
        const [addPackageResult] = await db.query(`
            ...
            VALUES (?, ?, ?)`, [cartId, package_id, quantity]);

        res.status(201).json({ message: 'Weinpaket hinzugefügt', cart_id: cartId, package_id, quantity });
    } catch (err) {
...
exports.updateCartItem = async (req, res) => {
    ...
    try {
        ...
        const [updateResult] = await db.query(`
            ...
            SET quantity = ?
            WHERE cart_id = ? AND package_id = ?`, [quantity, cartId, package_id]);

        if (updateResult.affectedRows === 0) {
            return res.status(404).json({ message: 'Weinpaket nicht im Warenkorb gefunden' });
        }
...
exports.removeFromCart = async (req, res) => {
    ...
    try {
        ...
        const [deleteResult] = await db.query(`
            ...
            WHERE cart_id = ? AND package_id = ?`, [cartId, package_id]);
