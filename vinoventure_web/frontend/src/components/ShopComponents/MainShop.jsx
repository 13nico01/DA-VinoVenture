    ...
    axios
      ...
      .then((response) => {
        ...
        if (Array.isArray(response.data.products)) {
          ...
          console.error("Erwartetes Array, aber keine erhalten:", response.data);
        }
      })
      .catch((error) => {
        ...
        {Array.isArray(products) ? (
          products.map((product) => (
            ...
            >
              ...
              />
              <div className="p-4">
                <h3 className="text-sm font-semibold text-gray-900">{product.package_name}</h3>
          ...
          <p className="text-center text-red-500">Keine Produkte gefunden oder falsche Datenstruktur</p>
        )}
      </div>
    </div>
  );
